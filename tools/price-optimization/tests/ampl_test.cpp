...
int main(int argc, char** argv) {
  ...
  try {
    ...
    for (int fixture = 0; fixture < 14; ++fixture) {
      ...
      if (enumerated.status != actual.status ||
          (actual.recommendation && (!enumerated.recommendation ||
           std::abs(enumerated.recommendation->expected_worker_surplus - actual.recommendation->expected_worker_surplus) > 1e-6))) {
      ...
      if (reference.status == BackendStatus::infeasible) {
        ...
        if (!actual.recommendation ||
            std::abs(actual.recommendation->expected_worker_surplus - reference.expected_worker_surplus) > 1e-6) {
    ...
    std::cout << "14 AMPL fixtures agree with exhaustive reference and bounded enumeration; AMPL presolve status 299 and solver infeasibility distinguished; "
