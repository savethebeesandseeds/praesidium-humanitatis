...
It accepts the [versioned contract](contract.md), runs the existing AMPL model
...
optimization objective is unchanged.
...
Every `result` identifies `ph.price.v1`, engine version, objective id/version,
model version and the compiled AMPL model's SHA-256. A parsed request also
...
| `infeasible` | AMPL/HiGHS reported infeasibility under the unchanged protected constraints. |
...
local affordability, change-cap and inventory violations; they do not claim
...
production-scale performance or operational AMPL entitlement. The dashboard
UI and additional store objectives remain separate work.
